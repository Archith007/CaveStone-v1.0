...
function preload() {
  ...
  chosenImage = loadImage("Screenshot 2021-02-01 at 9.43.44 AM.png")
...
function draw() {
  ...
  if (gameState === "pause") {
    ...
    var screen = createSprite(400, 475, 800, 800)
    screen.shapeColor = "white"
    pauseG.add(screen)

    var play = createSprite(405, 550)
    play.addImage(playImage)
    pauseG.add(play)

    var caveStoneB = createSprite(200, 250);
    caveStoneB.addAnimation("box1", blockAnimation)
    caveStoneB.scale = 2
    pauseG.add(caveStoneB)

    var coveStoneB = createSprite(600, 250);
    coveStoneB.addAnimation("box3", bubbleAnimation)
    coveStoneB.scale = 1
    pauseG.add(coveStoneB)


    var chosen = createSprite(210, 335)
  ...
  if (gameState === "play") {
    ...
    if (character === "coveStone") {
      ...
      if ((keyWentDown("DOWN_ARROW") || keyDown("S"))&& jump === 0) {
        block.velocityY = block.velocityY - 25
